...
def remove_course():
    try:
        ...
        # Kursu veritabanında ara
        cursor.execute("SELECT * FROM courses WHERE course_id=%s AND course_name=%s AND course_day=%s", 
                       (course_id, course_name, course_day))
        course = cursor.fetchone()

        # Kurs veritabanında bulunamazsa hata döndür
        if not course:
            return jsonify({'success': False, 'message': 'Course with the given ID, name, and day does not exist!'})
...
def remove_student():
    try:
        ...
        conn = get_db_connection()
        cursor = conn.cursor()
         # Öğrenci id'sini veritabanında ara
        cursor.execute("SELECT * FROM students WHERE student_id = %s", (student_id,))
        student = cursor.fetchone()

        # Öğrenci id'si veritabanında bulunamazsa hata döndür
        if not student:
            return jsonify({'success': False, 'message': 'Student with the given ID does not exist!'})
